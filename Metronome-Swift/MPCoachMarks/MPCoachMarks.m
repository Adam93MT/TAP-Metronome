...
static const CGFloat kCutoutRadius = 12.0f;
...
- (void)goToCoachMarkIndexed:(NSUInteger)index {
    ...
    CGFloat lblContinueWidth = self.bounds.size.width;//self.enableSkipButton ? (70.0/100.0) * self.bounds.size.width : self.bounds.size.width;
    CGFloat lblContinueHeight = 30.0f;
    CGFloat btnSkipWidth = self.bounds.size.width ;//- lblContinueWidth;
    
    // Show continue lbl if first mark
    if (self.enableContinueLabel) {
        if (markIndex == 0) {
            lblContinue = [[UILabel alloc] initWithFrame:(CGRect){{0, [self yOriginForContinueLabel]}, {lblContinueWidth, lblContinueHeight}}];
            ...
            lblContinue.textColor = [UIColor grayColor];
    ...
    if (self.enableSkipButton) {
        btnSkipCoach = [[UIButton alloc] initWithFrame:(CGRect){{0, [self yOriginForContinueLabel] + lblContinueHeight}, {btnSkipWidth, 30.0f}}];
